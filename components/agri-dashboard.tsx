  ...
  return (
    <main className="min-h-screen bg-[#f6faf7] text-slate-900">
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-12 lg:py-10">
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]"><Card className="rounded-[28px] border-0 bg-white shadow-[0_18px_50px_rgba(45,85,64,0.09)]"><CardHeader className="flex flex-row items-start justify-between px-6 pb-2 pt-6"><div><CardTitle className="text-lg tracking-tight">Smart diagnostics</CardTitle><p className="mt-1 text-xs text-slate-400">Most recent AI finding · 2 minutes ago</p></div><Badge className="rounded-full bg-rose-50 text-rose-600 hover:bg-rose-50"><Zap className="mr-1.5 size-3" /> Needs attention</Badge></CardHeader><CardContent className="px-6 pb-6 pt-4"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><div className="grid size-16 shrink-0 place-items-center rounded-2xl bg-rose-50 text-rose-500"><Leaf className="size-7" /></div><div className="min-w-0 flex-1"><div className="flex items-end justify-between gap-3"><div><p className="text-xl font-bold tracking-tight">Early Blight</p><p className="mt-1 text-xs text-slate-500">Tomato · North bed · Row 04</p></div><span className="text-2xl font-bold text-rose-500">96<span className="text-sm">%</span></span></div><Progress value={96} className="mt-3 h-2 bg-rose-100 [&>div]:bg-rose-500" /><div className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-900"><span className="font-bold">Treatment recommendation: </span>Isolate affected plants and apply an approved copper fungicide within 24 hours.</div></div></div></CardContent></Card><Card id="scans" className="rounded-[28px] border-0 bg-white shadow-[0_18px_50px_rgba(45,85,64,0.09)]"><CardHeader className="flex flex-row items-center justify-between px-6 pb-2 pt-6"><CardTitle className="text-lg tracking-tight">Recent scans</CardTitle><Button variant="ghost" size="sm" className="gap-1 rounded-lg text-xs text-slate-500">View all <ChevronRight /></Button></CardHeader><CardContent className="px-6 pb-5 pt-2">{scans.map((scan, index) => <div key={scan.label} className="flex items-center gap-3 py-3"><div className={`grid size-10 shrink-0 place-items-center rounded-xl ${scan.tone === 'rose' ? 'bg-rose-50 text-rose-500' : scan.tone === 'emerald' ? 'bg-emerald-50 text-emerald-500' : 'bg-amber-50 text-amber-500'}`}><Leaf className="size-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-sm font-semibold text-slate-800">{scan.label}</p><Badge variant="outline" className={`rounded-full text-[10px] ${scan.tone === 'rose' ? 'border-rose-100 text-rose-600' : scan.tone === 'emerald' ? 'border-emerald-100 text-emerald-600' : 'border-amber-100 text-amber-600'}`}>{scan.status}</Badge></div><p className="mt-0.5 text-[11px] text-slate-400">{scan.crop} · {scan.time}</p></div>{index < scans.length - 1 && <Separator className="absolute" />}</div>)}</CardContent></Card></section>
